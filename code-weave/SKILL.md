...
- ⚠️ **流程图节点的类名和方法名必须经过存在性验证**。在将 `类.方法名` 加入流程图前，必须使用以下命令在代码库中确认其存在（或通过 Read 工具读到该方法定义）：
  ```bash
  grep -r "方法名" --include="*.java" <项目路径>
  ```
  若 grep 无结果，**禁止**将该节点加入流程图；已加入但事后发现 grep 无结果的节点，必须立即删除并向用户说明。
...
2. **功能描述表格（必须）**：列出该节点的功能点。对于**校验逻辑**，表格的基础表头固定为（**必须包含「序号」列，从 1 开始顺序编号**）：
...
**当一个大的校验节点下包含多个小的校验子节点时**，使用层级展开：大节点用 `## 2.X`，子节点用 `### 2.X.Y`。大节点按上述四步顺序展开，其中一句话总结功能为必须项，其余为可选。子节点同样按上述四步顺序完整展开。
...
- **全局特殊条件**：影响整个流程的条件或场景，写在「一、整体流程图」的流程图之后。例如：
  - **登录要求**：需要用户登录后才能执行
  - **并发控制**：并发场景下使用分布式锁机制
...
对流程图中每个 `类.方法名` 格式的节点，执行存在性验证（与 Step 3 要求一致）：
```bash
grep -r "方法名" --include="*.java" <项目路径>
```
...
- **流程图中只统计有业务功能的节点**。忽略所有无业务判断的节点，包括：纯数据查询（`queryXxx`/`getXxx`）、初始化/上下文准备（`init`/`setUp`）、数据分组/聚合、结果合并（`mergeResult`）、结果构建/格式转换（`buildXxx`/`convertXxx`）、return 语句/终止节点、纯异常捕获、类型转换、序列化/反序列化。详见「绘制流程图」规则。
